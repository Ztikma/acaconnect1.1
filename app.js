...
const PLACEHOLDER_IMGS = { 
  gastronomia: 'https://images.unsplash.com/photo-1504544750208-dc0358e411f5?w=600', 
  hospedaje: 'https://images.unsplash.com/photo-1520250497591-112f2f40a3f4?w=600', 
  servicios: 'https://images.unsplash.com/photo-1600880292203-757bb62b4baf?w=600', 
  experiencias: 'https://images.unsplash.com/photo-1505118380757-91f5f5632de0?w=600' 
};
...
async function loadServices() {
  ...
  try {
    ...
    grid.innerHTML = '<div class="empty-state">⚠️ Error cargando servicios</div>'; 
  }
}
...
function renderCards() {
  ...
  if (!filtered.length) { 
    grid.innerHTML = '<div class="empty-state">🔍 No se encontraron servicios.</div>'; 
    return; 
  }
...
async function publishService() {
  ...
  const titulo = document.getElementById('pub-title').value; 
  const precio = document.getElementById('pub-price').value; 
  ...
  try {
    ...
    await supaFetch('/rest/v1/servicios', {
      ...
      body: JSON.stringify({ 
        ...
        precio_tipo: document.getElementById('pub-price-type').value, 
    ...
    btn.textContent = 'Publicar Servicio'; 
    btn.disabled = false; 
  }
}
...
async function fetchAcapulcoWeather() {
  try {
    ...
    document.getElementById('weather-temp').textContent = '29°'; 
  }
}
...
async function doRegister() {
  ...
  try {
    ...
    showToast('❌ Correo ya en uso o error'); 
  }
}
...
async function loadResenas(servicioId) {
  ...
  if (currentUser) {
    ...
    formArea.innerHTML = `<div class="login-prompt"><p>Inicia sesión para reseñar</p><button class="btn-login-resena" onclick="openModal('login')">Entrar</button></div>`; 
  }
...
function renderResenas(resenas, servicioId) {
  ...
  if (!resenas.length) { 
    ...
    list.innerHTML = '<div class="resenas-empty">Sin reseñas aún.</div>'; 
    return; 
  }
  ...
  document.getElementById('resena-total').textContent = resenas.length + ' reseñas';
...
function pickStar(n) { 
  selectedStars = n; 
  document.querySelectorAll('.star-pick').forEach(s => s.classList.toggle('selected', parseInt(s.dataset.val) <= n)); 
}
...
async function submitResena(servicioId) {
  ...
  try {
    ...
    showToast('Ya reseñaste esto o hubo error'); 
    btn.disabled = false; 
...
async function openMisPub() {
  ...
  try {
    ...
    if (!pubs.length) { 
      document.getElementById('mispub-content').innerHTML = '<div class="mispub-empty">No tienes publicaciones.</div>'; 
      return; 
    }
    ...
    document.getElementById('mispub-content').innerHTML = '<div class="mispub-empty">Error al cargar.</div>'; 
  }
}
...
async function openAdmin() {
  console.log("Abriendo panel de administrador...");
  
  if (!currentUser) {
    console.log("Acceso denegado: No hay sesión iniciada");
    return showToast('Acceso denegado');
  }
  
  if (currentUser.tipo !== 'admin') {
    console.log("Acceso denegado: El usuario es", currentUser.tipo);
    return showToast('Solo para administradores');
  }
  
  const adminPanel = document.getElementById('admin-panel');
  if (!adminPanel) {
    console.error("Error: No se encontró el div #admin-panel en el HTML.");
    return;
  }
  ...
  document.querySelectorAll('.admin-tab').forEach(t => {
    t.classList.toggle('active', t.dataset.tab === 'pendientes');
  });
...
async function loadAdminList(tab) {
  ...
  try {
    if(tab === 'usuarios') {
      ...
      if(!data.length) { 
        list.innerHTML = `<div class="empty-state">No hay servicios ${estado}s</div>`; 
        return; 
      }
...
async function toggleAdmin(userId, tipo) {
  ...
  try { 
    await supaFetch('/rest/v1/usuarios?id=eq.'+userId, { method: 'PATCH', body: JSON.stringify({ tipo: n }) }); 
    loadAdminList('usuarios'); 
    showToast('Rol actualizado'); 
  } catch(e) {
    showToast('Error actualizando rol');
  }
}
...
document.addEventListener('click', (e) => {
  if (e.target && e.target.id === 'theme-toggle') {
    ...
    if (isDark) { 
      ...
      localStorage.setItem('aca_theme', 'light'); 
      e.target.textContent = '🌙'; 
    } else { 
      document.documentElement.setAttribute('data-theme', 'dark'); 
      localStorage.setItem('aca_theme', 'dark'); 
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (localStorage.getItem('aca_theme') === 'dark') { 
    document.documentElement.setAttribute('data-theme', 'dark'); 
  ...
  // 3. Cargar la página correcta
  const hash = window.location.hash.replace('#', '') || 'home'; 
  showPage(hash, false);
  
  // 4. Iniciar animadores
  initStatsObserver();
});
